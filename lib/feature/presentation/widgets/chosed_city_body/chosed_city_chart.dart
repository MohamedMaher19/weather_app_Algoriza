...
class ChosedCityChart extends StatelessWidget {
  const ChosedCityChart({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<WeatherBloc,WeatherState>(
        builder: (context,state){
          switch (state.hourlyRequestForChosedCityState){
            ...
            case RequestState.loaded:
              return Container(
                ...
                child: SfCartesianChart(
                  ...
                  series: <ChartSeries>[
                    LineSeries<HourlyWeatherForChosedCity,String>(
                      dataSource: state.hourlyWeatherForChosedCity,
                      xValueMapper: (HourlyWeatherForChosedCity weather,_) => DateFormat('h:mm a').format(DateTime.fromMillisecondsSinceEpoch(weather.time*1000,isUtc: false)),
                      yValueMapper: (HourlyWeatherForChosedCity weather,_) => weather.temperature,
